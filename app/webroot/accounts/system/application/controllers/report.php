...
class Report extends Controller {
	...
	function cakephp(){
		$baseUrl = str_replace('app/webroot/accounts/', '', $this->config->config['base_url']);
		$istoreURL = $baseUrl.$this->uri->segments[3].'/'.@$this->uri->segments[4];
	
		$this->template->set('page_title', '');
		$this->template->set('page_sidebar', 'Add New');
		
		$this->template->load('template', 'report/istoreview', array('istoreURL' => $istoreURL));
		return;
	}
