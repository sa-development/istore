<div class="kitparts index">
	<h2><?php echo __('Kitparts'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('product_id'); ?></th>
			<th><?php echo $this->Paginator->sort('part_id'); ?></th>
			<th><?php echo $this->Paginator->sort('qty'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php
	foreach ($kitparts as $kitpart): ?>
	<tr>
		<td><?php echo h($kitpart['Kitpart']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($kitpart['Product']['product_no'], array('controller' => 'products', 'action' => 'view', $kitpart['Product']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($kitpart['Part']['id'], array('controller' => 'parts', 'action' => 'view', $kitpart['Part']['id'])); ?>
		</td>
		<td><?php echo h($kitpart['Kitpart']['qty']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $kitpart['Kitpart']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $kitpart['Kitpart']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $kitpart['Kitpart']['id']), null, __('Are you sure you want to delete # %s?', $kitpart['Kitpart']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>

	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Kitpart'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Products'), array('controller' => 'products', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Product'), array('controller' => 'products', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Parts'), array('controller' => 'parts', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Part'), array('controller' => 'parts', 'action' => 'add')); ?> </li>
	</ul>
</div>
